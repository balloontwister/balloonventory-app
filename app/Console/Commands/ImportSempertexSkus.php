...
class ImportSempertexSkus extends Command
{
    ...
    protected $description = 'Import Sempertex SKUs from intake/sempertex/sempertex_normalized.json';

    public function handle(): int
    {
        $dryRun = ! $this->option('execute');
        $jsonPath = $this->option('path') ?: base_path('intake/sempertex/sempertex_normalized.json');

        if (! file_exists($jsonPath)) {
            $this->error("File not found: {$jsonPath}");
            $this->line('Run: python3 intake/sempertex/sempertex_normalize.py');
