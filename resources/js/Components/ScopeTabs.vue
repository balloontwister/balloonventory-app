...
const activeList = computed(() =>
    props.activeListId ? props.lists.find((l) => l.id === props.activeListId) : null,
);
</script>
...
<template>
    <div class="flex flex-col gap-2">
        <!-- tab bar -->
        <div class="relative inline-flex items-center rounded-pill bg-background p-1">
            <!-- All -->
            <button
                type="button"
            ...
            <div class="relative">
                ...
                >
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12" fill="currentColor" class="h-3 w-3">
                        <path fill-rule="evenodd" d="M3.22 4.72a.75.75 0 011.06 0L6 6.44l1.72-1.72a.75.75 0 111.06 1.06L6.53 8.03a.75.75 0 01-1.06 0L3.22 5.78a.75.75 0 010-1.06z" clip-rule="evenodd" />
                    </svg>
                </button>
                ...
                >
                    ...
                    >
                        ...
                        <span class="font-mono text-[12px] text-ink-secondary">{{ list.sku_count }}</span>
                    </button>
                ...
                <div v-if="listsOpen" class="fixed inset-0 z-30" @click="listsOpen = false" />
            </div>
        </div>

        <!-- active list label -->
        <div v-if="activeScope === 'list' && activeList" class="flex items-center gap-3">
            <h2 class="font-display text-[24px] font-semibold leading-[1.2] tracking-h2 text-ink-primary">
                {{ activeList.name }}
            </h2>
            <span class="font-mono text-[14px] text-ink-secondary">{{ activeList.sku_count }} SKUs</span>
            <a
