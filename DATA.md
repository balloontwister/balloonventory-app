...
- **Soft delete + unique constraints**: in MariaDB, a unique constraint includes soft-deleted rows by default. Where a tombstoned row would block re-creation (e.g. `unique(list_id, sku_id)` on `list_item`), include `deleted_at` in the unique composite: `unique(list_id, sku_id, deleted_at)`. MariaDB treats NULL as not-equal-to-itself in unique indexes, so multiple soft-deleted rows can coexist while only one active row is allowed. This is the inverse of Postgres partial-index handling, which is why this convention is explicit.
