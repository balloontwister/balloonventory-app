...
class ImportGemarSkus extends Command
{
    protected $signature = 'catalog:import-gemar
    ...
    public function handle(): int
    {
        ...
        foreach ($rows as $row) {
            ...
            if (strlen($barcode) === 12 && Gtin::isValidCheckDigit($barcode)) {
                ...
                $warnings[] = "Invalid barcode '{$barcode}' for '{$row['warehouse_sku']}'";
            }

            $existing = $existingByWarehouseSku->get($row['warehouse_sku']);

            if ($existing) {
                // No-clobber: check barcode conflicts
                if (isset($attrs['upc']) && $existing->upc && $existing->upc !== $attrs['upc']) {
                    $barcodeConflicts[] = "UPC conflict for '{$row['warehouse_sku']}': existing={$existing->upc} incoming={$attrs['upc']}";
                    unset($attrs['upc']);
                }
                if (isset($attrs['ean']) && $existing->ean && $existing->ean !== $attrs['ean']) {
                    $barcodeConflicts[] = "EAN conflict for '{$row['warehouse_sku']}': existing={$existing->ean} incoming={$attrs['ean']}";
                    unset($attrs['ean']);
                }

                // No-clobber: never overwrite existing barcode with nothing
                if ($existing->upc) {
                    unset($attrs['upc']);
                }
                if ($existing->ean) {
        ...
        if ($barcodeConflicts) {
            ...
            if (count($barcodeConflicts) > 10) {
                $this->warn('  ... and '.(count($barcodeConflicts) - 10).' more');
            }
        }
    ...
    /**
     * Group every Gemar SKU by (balloon_size_id, color_id). For every group
