...
<template>
    <!-- Read-only: non-interactive star for users without favorites.edit -->
    <span
        v-if="readonly"
        class="flex h-8 w-8 items-center justify-center"
        :title="isFavorite ? 'Favorited' : ''"
    >
        <svg
            v-if="isFavorite"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            class="h-5 w-5 text-accent opacity-60"
            fill="currentColor"
        >
            <path
    ...
    <!-- Interactive star for users with favorites.edit -->
    <button
        v-else
        type="button"
