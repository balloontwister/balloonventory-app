...
function runBackup() {
    ...
    router.post(route('super-admin.backups.store'), {}, {
...
<template>
    ...
    <AuthenticatedLayout>
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="mb-1 text-sm text-ink-tertiary">
                        <a :href="route('super-admin.dashboard')" class="hover:underline">
        ...
        <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
            <!-- Empty state -->
            <div v-if="backups.length === 0" class="rounded-xl border border-border bg-surface py-16 text-center text-ink-tertiary">
            ...
            <div v-else class="overflow-hidden rounded-xl border border-border bg-surface">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-border bg-background text-left text-xs font-semibold uppercase tracking-wide text-ink-tertiary">
                            <th class="px-4 py-3">{{ $t('super_admin.backups.col_filename') }}</th>
                            <th class="px-4 py-3">{{ $t('super_admin.backups.col_size') }}</th>
                            <th class="px-4 py-3">{{ $t('super_admin.backups.col_created') }}</th>
                            <th class="px-4 py-3 text-right">{{ $t('super_admin.backups.col_actions') }}</th>
                        </tr>
                    </thead>
